...
function fresh(){return{version:2,revision:0,settings:{focus:['や','せ'],pool:[],guide:false},cursor:0,focusCursor:0,cards:[],sessions:[],activeId:null};}
...
function start(state){
 let route=[];const {pool,focus}=state.settings;
 if(pool.length){route=cycle(pool,state.cursor,5);state.cursor+=5;}
 else {const f=focus.length?focus:[],rest=CHARS.filter(c=>!f.includes(c));const chosen=cycle(f,state.focusCursor,Math.min(2,f.length));state.focusCursor+=chosen.length;const other=cycle(rest,state.cursor,Math.min(5-chosen.length,rest.length));state.cursor+=other.length;
  route=[chosen[0],other[0],chosen[1],...other.slice(1)].filter(Boolean);const missing=CHARS.filter(c=>!route.includes(c));while(route.length<5)route.push(missing.shift()||CHARS.find(c=>c!==route.at(-1)));}
 const s={id:uid(),createdAt:new Date().toISOString(),questions:route.map(question),guide:state.settings.guide,index:0,reward:null,repeat:false};state.sessions.push(s);state.sessions=state.sessions.slice(-100);state.activeId=s.id;return s;
}
...
function validate(x,counts){
 ...
 check(x&&x.version===2,'対応していない保存形式です');check(integer(x.revision,1e9)&&integer(x.cursor,1e9)&&integer(x.focusCursor,1e9),'巡回位置が不正です');
 check(x.settings&&typeof x.settings.guide==='boolean','設定が不正です');for(const k of ['focus','pool'])check(Array.isArray(x.settings[k])&&x.settings[k].length<=7&&unique(x.settings[k])&&x.settings[k].every(c=>CHARS.includes(c)),'文字の設定が不正です');
 ...
 for(const s of x.sessions){check((s.guide===undefined||typeof s.guide==='boolean')&&str(s.id)&&str(s.createdAt)&&Number.isFinite(Date.parse(s.createdAt))&&integer(s.index,5)&&typeof s.repeat==='boolean'&&(s.reward===null||str(s.reward)),'セッションが不正です');check(Array.isArray(s.questions)&&s.questions.length===5,'出題数が不正です');
  ...
  if(s.reward)check(s.index===5&&s.questions.every(q=>q.complete)&&x.cards.includes(s.reward),'報酬が不整合です');else check(s.index<5,'報酬未確定の完了記録です');
 }
...
const api={CHARS,START_TOLERANCE,PATH_TOLERANCE,fresh,start,active,firstRecord,assist,uncertain,award,validate,dist,candidates,advance,resolve};if(typeof module!=='undefined')module.exports=api;else root.RailCore=api;
})(globalThis);
