...
const HIRAGANA=['や','せ','ふ','ほ','も','む','よ','い','つ','し','く','へ','り','こ','う','て','と','に','か','た','な','の'];
...
assert.equal(courses.hiragana.length,22);
assert.equal(courses.kanji.length,17);
assert.equal(cards.length,30);

console.log('PASS: canonical courses, 39 character shapes, 30 cards, ordering, uniqueness, required fields, and image references');
