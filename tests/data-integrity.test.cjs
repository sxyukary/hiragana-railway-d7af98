...
const HIRAGANA=['や','せ','ふ','ほ','も','む','よ','い','つ','し','く','へ','り','こ','う','て','と','に','か','た','な','の','あ','え','お','き','け','さ','ち','は','ひ','ま','み','ゆ','わ','ら','る','れ','す','そ','ぬ','ね','め','ろ','を','ん'];
...
assert.equal(courses.hiragana.length,46);
assert.equal(courses.katakana.length,46);
assert.equal(courses.kanji.length,22);
assert.equal(cards.length,40);

console.log('PASS: canonical courses, 114 character shapes, 40 cards, ordering, uniqueness, required fields, and image references');
